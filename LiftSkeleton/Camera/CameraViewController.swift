...
final class CameraViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        overlayView.isFrontCamera = (cameraManager.currentPosition == .front)

        cameraManager.onFrame = { [weak self] sampleBuffer in
            self?.handle(sampleBuffer: sampleBuffer)
        }

        cameraManager.onRecordingStateChanged = { [weak self] isRecording in
    ...
    func switchCamera() {
        cameraManager.switchCamera { [weak self] success in
            ...
            DispatchQueue.main.async {
                self.overlayView.isFrontCamera = (self.cameraManager.currentPosition == .front)
                self.overlayView.observation = nil
            }
        }
    }
    ...
    private func handle(sampleBuffer: CMSampleBuffer) {
        ...
        let orientation: CGImagePropertyOrientation =
            (cameraManager.currentPosition == .front) ? .rightMirrored : .left

        let observation = poseDetector.detectPose(in: sampleBuffer, orientation: orientation)
